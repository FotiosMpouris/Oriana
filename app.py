...
if 'source_to_remove' not in st.session_state:
    st.session_state.source_to_remove = None
...
for source in oriana.sources:
    ...
    if col2.button("X", key=f"remove_{source}"):
        st.session_state.source_to_remove = source
        st.rerun()

# Confirmation for source removal
if st.session_state.source_to_remove:
    if st.sidebar.button(f"Remove {st.session_state.source_to_remove}?"):
        oriana.remove_source(st.session_state.source_to_remove)
        st.sidebar.success(f"Removed source: {st.session_state.source_to_remove}")
        st.session_state.source_to_remove = None
        st.rerun()
